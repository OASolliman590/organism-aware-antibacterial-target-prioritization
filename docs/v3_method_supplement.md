...
sensitivity analysis of both the transparent chemical rank fusion and the final
organism-aware heuristic ranking.
...
A separate final-ranking report perturbs by the same factors every configured
coefficient in the target-specificity transform and reference-quality mapping,
then the affine species-transfer, pocket-evidence, biological-priority, and
anti-target terms; each layer is also removed in turn. It additionally perturbs
and ablates each organism-scope, clinical, essentiality, accessibility,
resistance, and CARD context subweight used to construct biological priority.
The configured chemical-quality and final-ranking formulae must reproduce the
recorded baseline scores before this analysis proceeds.
Kendall tau and RBO are calculated per organism-query ranked list; 95% intervals
resample those complete ranked lists with the pinned bootstrap seed. Rows with
missing component evidence are counted and excluded, never imputed. The output
`final_ranking_sensitivity_v3.csv` and its figure remain non-probabilistic and are
reported separately from the reference-ligand bootstrap of chemical fusion.
...
`pipeline/sensitivity_analysis.py`: chemical fusion uses the reference-bootstrap
table, while final-ranking sensitivity uses the organism-query grouped bootstrap
described above.
